    ...
    <div className={`rounded-2xl font-sans ${className}`} style={style}>
      <div className={headerClassName ?? 'px-5 pt-5 pb-3 flex items-center gap-3'}>
        ...
        <div className="flex flex-col">
          <span
            className={labelClassName ?? 'text-[13px] uppercase tracking-[0.14em] font-semibold'}
            style={labelStyle}
          >
            {label}
          </span>
      ...
      <div className={bodyClassName ?? 'px-5 pb-5 h-full text-[15px] leading-relaxed'}>{children}</div>
    </div>
  );
};
