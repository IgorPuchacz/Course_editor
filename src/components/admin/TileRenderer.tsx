...
export const TileRenderer: React.FC<TileRendererProps> = ({
  ...
  const isTaskTile = tile.type === 'programming' || tile.type === 'sequencing';
  let taskTileSurfaceStyle: React.CSSProperties | undefined;
  let taskTileShowsBorder: boolean | undefined;
  ...
  const renderTileContent = () => {
    ...
    switch (tile.type) {
      ...
      case 'programming': {
        ...
        taskTileSurfaceStyle = {
          backgroundColor: accentColor,
          backgroundImage: `linear-gradient(135deg, ${gradientStart}, ${gradientEnd})`,
          color: textColor,
        };
        taskTileShowsBorder = programmingTile.content.showBorder !== false;
        if (taskTileShowsBorder) {
          taskTileSurfaceStyle.borderColor = containerBorderColor;
        } else {
          taskTileSurfaceStyle.border = 'none';
        }
        ...
        if (isEditingText && isSelected) {
          contentToRender = (
            <div className="flex h-full flex-col gap-5 p-5" style={{ color: textColor }}>
              ...
              <div
                className="flex flex-1 flex-col overflow-hidden rounded-2xl border backdrop-blur-sm transition-colors duration-300"
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3">
                    <button
                      className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-500/90 transition-colors shadow-lg shadow-emerald-500/30 hover:bg-emerald-500"
                      title="Uruchom kod"
                    ...
                    <button
                      className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-500/90 transition-colors shadow-lg shadow-rose-500/30 hover:bg-rose-500"
                      title="Zatrzymaj kod"
                      onClick={(e) => e.stopPropagation()}
                    >
                ...
                <div className="relative flex-1">
                  <pre
                    className="h-full w-full overflow-auto whitespace-pre px-14 py-6 font-mono text-sm leading-loose text-emerald-400"
                    style={{
                  ...
                  <div className="pointer-events-none absolute left-5 top-6 select-none text-xs font-mono leading-relaxed" style={{ color: 'rgba(148, 163, 184, 0.55)' }}>
                    {codeLines.map((_, index) => (
                      <div key={index} className="flex h-[1.65em] items-center">
                        <span>{index + 1}</span>
                      </div>
                    ))}
                  </div>
          ...
          contentToRender = (
            <div className="flex h-full flex-col gap-5 p-5" style={{ color: textColor }}>
              {renderDescriptionBlock(
                <div
                  className="rich-text-content tile-formatted-text h-full w-full overflow-auto break-words"
                  style={{
                    minHeight: '1em',
                    outline: 'none',
                    color: textColor,
      ...
      case 'sequencing': {
        ...
        taskTileShowsBorder = sequencingTile.content.showBorder !== false;
        if (taskTileShowsBorder) {
          taskTileSurfaceStyle.borderColor = borderColor;
        } else {
          taskTileSurfaceStyle.border = 'none';
        }
  ...
  const tileBody = renderTileContent();

  const tileContentClasses = [
    'w-full',
    'h-full',
    isProgrammingTile ? 'overflow-visible' : 'overflow-hidden'
  ];

  if (isTaskTile) {
    tileContentClasses.push('rounded-lg', 'shadow-sm');
    if (taskTileShowsBorder === false) {
      tileContentClasses.push('border-0');
    } else {
      tileContentClasses.push('border');
    }
  } else if (!isFramelessTextTile) {
    tileContentClasses.push('bg-white', 'border', 'border-gray-200', 'shadow-sm', 'rounded-lg');
  }

  const tileContentStyle: React.CSSProperties | undefined = isTaskTile
    ? { ...(taskTileSurfaceStyle ?? {}) }
    : isFramelessTextTile
      ? {
  ...
  if (isTaskTile && tileContentStyle && taskTileShowsBorder === false) {
    tileContentStyle.border = 'none';
  }
  ...
  return (
    ...
    >
      {/* Tile Content */}
      <div
        className={tileContentClasses.join(' ')}
